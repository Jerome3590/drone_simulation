---
title: "COVID-19 Telehealth Simulation: New Mexico Case Study"
format:
  html:
    toc: true
    df-print: paged
...
[Webscraping Demo](https://iam-lab-analytics-summit.s3.amazonaws.com/nm_covid_data.html)
...
internet_5G <- function(){return(sample(c(0,1), size=1, replace = T, c(.8,.2)))}
...
drone_flight_time_supply <- function(){runif(1,.5,1)}
...
simulate <- function(param) {
  ...
  add_resource(drones_supply, capacity = 5, mon = TRUE) %>% 
  add_resource(beds, capacity = 100, mon = TRUE) %>% 
  add_resource(no_internet_zones, capacity = 1500, mon = TRUE) %>% 
  add_resource(drones_5G, capacity = 5000, mon = TRUE) 
  
  env %>% 
  add_global("Total_Hospitalization_Cost", 0) %>% 
  add_global("Total_Telehealth_Cost", 0) 
  ...
  set_global("Total_Telehealth_Cost", mod="+", 600) %>%
...
# grid of scenarios
cases <- expand.grid(supply_drones_n = c(1,5,10,20,30,40), 
                     sim_time = c(4,12,16,32,52), 
                     `5G_drones_n` = c(1,2))
...
```{r Visualize-Scenario-Cargo-Drones, warning=FALSE, message=FALSE}
...
```{r google-drive-setup, eval=FALSE}

gdrive_scenario_data <- drive_find(pattern="scenario_data")

saveRDS(gdrive_scenario_data2, file = "gdrive.rds")

```


```{r google-drive-upload, eval=FALSE}
...
```{sh upload-website-dependencies}
