...
patient_attributes <- zip_plus4_geocodio %>% 
  ...
  left_join(.,
            hospitals,
            by="County") %>% 
  mutate(patient_id = paste0(`Zip+4`,"_", rownames(zip_plus4_geocodio))) %>% 
  select(16,1:15)
...
ages <- c(30:105)
age <- function(){return(sample(ages, size=1, replace=TRUE))}

genders <- c(1:3)
gender <- function(){return(sample(genders, size=1, replace=TRUE, c(.45,.5,.05)))}

ethnicity <- c(1:5)
race <- function(){return(sample(ethnicity, size=1, replace=TRUE, c(.4,.3,.15,.1,.05)))}
...
insurance <- function(){return(sample(insurance_policies, size=1, replace=TRUE))}
...
s3write_using(patient_attributes, FUN = write.csv,
                    bucket = "irad-2201-scenario-modeling",
                    object = "simulation/patients/patient_attributes.csv")
